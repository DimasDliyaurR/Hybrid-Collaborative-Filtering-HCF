...
class MatrixRating():
    ...
    def __dozenOfItem(self) -> dict :
        ...
        return {
            "unrated" : [[[ j for j in range(len(self.train[indexFold][i])) if self.train[indexFold][i][j] == 0] for i in range(len(self.train[indexFold]))] for indexFold in range(len(self.train))],
            "rated" : [[[ j for j in range(len(self.train[indexFold][i])) if self.train[indexFold][i][j] != 0] for i in range(len(self.train[indexFold]))] for indexFold in range(len(self.train))],
        }
    ...
    def __dozenOfUser(self) -> dict[str : Matrix ] :
        ...
        return {
            "unrated" : [[[ j for j in range(len(self.train_transpose[indexFold][i])) if self.train_transpose[indexFold][i][j] == 0] for i in range(len(self.train_transpose[indexFold]))] for indexFold in range(len(self.train))],
            "rated" : [[[ j for j in range(len(self.train_transpose[indexFold][i])) if self.train_transpose[indexFold][i][j] != 0] for i in range(len(self.train_transpose[indexFold]))] for indexFold in range(len(self.train))],
        }

    def __dozenOfItemTest(self) -> dict :
        return {
            "unrated" : [[[ j for j in range(len(self.test[indexFold][i])) if self.test[indexFold][i][j] == 0] for i in range(len(self.test[indexFold]))] for indexFold in range(len(self.test))],
            "rated" : [[[ j for j in range(len(self.test[indexFold][i])) if self.test[indexFold][i][j] != 0] for i in range(len(self.test[indexFold]))] for indexFold in range(len(self.test))],
        }

    def __dozenOfUserTest(self) -> dict[str : Matrix ] :
        return {
            "unrated" : [[[ j for j in range(len(self.test_transpose[indexFold][i])) if self.test_transpose[indexFold][i][j] == 0] for i in range(len(self.test_transpose[indexFold]))] for indexFold in range(len(self.test))],
            "rated" : [[[ j for j in range(len(self.test_transpose[indexFold][i])) if self.test_transpose[indexFold][i][j] != 0] for i in range(len(self.test_transpose[indexFold]))] for indexFold in range(len(self.test))],
        }
    ...
    def getUniqueIdOfUserTrain(self,*,indexFold : None|int = None) :
        return self.uniqueIdOfUserAndItemTrain["users"][indexFold]
    
    def getUniqueIdOfItemTrain(self,*,indexFold : None|int = None) :
        return self.uniqueIdOfUserAndItemTrain["items"][indexFold]
    
    def getUniqueIdOfUserTest(self,*,indexFold : None|int = None) :
        return self.uniqueIdOfUserAndItemTest["users"][indexFold]
    
    def getUniqueIdOfItemTest(self,*,indexFold : None|int = None) :
        return self.uniqueIdOfUserAndItemTest["items"][indexFold]
        


    def getItem(self, user : int, indexFold : int|None = None,*, interacted : bool = True) -> Vector :
        """
        ...
        if self.toyData :
            ...
            if user > len(self.__matrix_component_of_item[label][indexFold]) :
                raise ValueError("Data akses melebihi matriks")

            return self.__matrix_component_of_item[label][indexFold][user]

    def getUser(self ,item : int,indexFold : int|None = None,*,interacted: bool|None = True) -> Vector :
        """
        ...
        if self.toyData :
            ...
            if interacted is None :
                return list( set(self.__matrix_component_of_user["rated"][item]) | set(self.__matrix_component_of_user["unrated"][item]) )
            ...
            if item > len(self.__matrix_component_of_user[label][indexFold]) :
                raise ValueError("Data akses melebihi matriks")
            
            if interacted is None :
                return list( set(self.__matrix_component_of_user["rated"][indexFold][item]) | set(self.__matrix_component_of_user["unrated"][indexFold][item]) )
            
            return self.__matrix_component_of_user[label][indexFold][item]
    
    def getItemTest(self, user : int, indexFold : int|None = None,*, interacted : bool|None = True) -> Vector :
        """
        Get set of item have rated by specific user
            Representation of I_u or \widehat{I}_u (depend on parameter interacted) Notation
        ------------------------------------------------------------------------------------
        ...
        if self.toyData :
            ...
            if indexFold is None :
                raise ValueError(f"Index of train is missing {self}")
            return self.__matrix_component_of_item_test[label][indexFold][user]

    def getUserTest(self ,item : int,indexFold : int|None = None,*,interacted: bool|None = True) -> Vector :
        """
        Get set of user have rated by specific item
            Representation of U_i or \widehat{U}_i (depend on parameter interacted) Notation
        ------------------------------------------------------------------------------------
        ...
        if self.toyData :
            ...
            if interacted is None :
                return list( set(self.__matrix_component_of_user_test["rated"][item]) | set(self.__matrix_component_of_user_test["unrated"][item]) )
            ...
            if interacted is None :
                return list( set(self.__matrix_component_of_user_test["rated"][indexFold][item]) | set(self.__matrix_component_of_user_test["unrated"][indexFold][item]) )
            
            return self.__matrix_component_of_user_test[label][indexFold][item]

    def getItemWithValue(self,user : int, *,indexFold : int|None = None) -> Vector :
        
        if self.toyData :
            return self.matrixRating[user]
        else :
            return self.train[indexFold][user]

    def getUserWithValue(self, item : int, indexFold : int|None = None) -> Vector :
        
        if self.toyData :
            return self.reverseMatrixRating[item]
        else :
            return self.train_transpose[indexFold][item]
